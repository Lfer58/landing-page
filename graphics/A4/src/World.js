...
function renderWalls(object, cornerPos, scale) {
  ...
  object.normalMatrix.setInverseOf(object.matrix).transpose();
  object.render();
}
...
function renderAllShapes() {
  ...
  // pass the light pos into GLSL
  gl.uniform3f(u_lightPos, ...g_LightPos);
  gl.uniform3f(u_spotlightPos, ...g_LightPos);

  gl.uniform3f(u_spotlightDir, ...g_spotlightDir)

  gl.uniform1f(u_spotlightCutoff, 30)
  gl.uniform1f(u_spotlightExponent, 10)

  gl.uniform3f(u_lightColor, ...g_color)

  // pass the camera pos into GLSL
  gl.uniform3f(u_cameraPos, camera.g_eye[0], camera.g_eye[1], camera.g_eye[2]);

  // draw light
  cornerPos = [-0.5, -0.5, -0.5]
  scale = [-0.1,-0.1,-0.1]
  var light = new CubeMod_Rep([2,2,0,1], ...scale, 1, -2);
  light.matrix.translate(g_LightPos[0], g_LightPos[1], g_LightPos[2]);
  light.matrix.scale(...scale);
  light.render();

  cornerPos = [0,0,0]
  scale = [1,1,1]
  var sphere = new Sphere(base_color, ...scale, 1, 0);
  renderWalls(sphere, cornerPos, scale);
  ...
  cornerPos = [100, -50, 100]
  scale = [-200, 200, -200]
  var skybox = new CubeMod_Rep(base_color, 1,1,1, 1, 1);
  renderWalls(skybox, cornerPos, scale)

  cornerPos = [-100, 100, -100]
  scale = [200, 0, 200]
  var sky = new CubeMod_Rep(base_color, 1, 1, 1, 1, 2);
  renderWalls(sky, cornerPos, scale)
